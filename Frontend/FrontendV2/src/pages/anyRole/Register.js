...
const Register = () => {
  ...
  let emailStudent = new RegExp(/^(([^<>()\[\]\\.,;:\s@"]+(\.[^<>()\[\]\\.,;:\s@"]+)*)|(".+"))@student.kuleuven.be/)
  ...
  const [researchGroupList, setResearchGroupList] = useState([]);
    ...
    const onSubmit = (data) => {
      ...
      if (role == "ROLE_PROMOTOR"){
          ...
          axios
              .post("http://localhost:8080/promotors/register", promotor)
              .then((response) => {
                  console.log(response);
                  alert("User Added Successfully")
              }, (error) => {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <label htmlFor="role">role</label>
          <select onChange={(e) => setRole(e.target.value)}>
              <option selected={"default"} disabled hidden>Select a role</option>
